...
dim(label)
#label[1:10,1]
write.csv(label,file = 'C:\\Users/idehz/Desktop/Methylation/label_seq_name1.csv')
#label[,1]
for(m in 1:length(label[,1]))
{
    file_name = file(paste0('C:\\Users/idehz/Desktop/Methylation/Seq1/',as.character(label[m,1]),'.lab'))
    writeLines(as.character(label[m,3]),sep = "\n",file_name)
    close(file_name)
}
for(m in 1:length(label[,1]))
{
    file_name = file(paste0('C:\\Users/idehz/Desktop/Methylation/Seq1/',as.character(label[m,1]),'.name.lab'))
    writeLines(c(paste0('>',label[m,1]),as.character(label[m,3])),sep = "\n",file_name)
    close(file_name)
}
for(m in 1:length(label[,1]))
{
    file_name = file(paste0('C:\\Users/idehz/Desktop/Methylation/Seq1/',as.character(label[m,1]),'.name.seq.lab'))
